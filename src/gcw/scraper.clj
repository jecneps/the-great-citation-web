...
(defn update-in-network-links [network-urls post-entry]
  (let [in-network (into #{} (:in-network-links post-entry))
        network-hosts (->> (map uri/parse network-urls)
                           (map :host)
                           (into #{}))
        out-network (into #{} (:out-of-network-links post-entry))
        new-in-network (filter #(contains? network-hosts
                                           (:host (uri/parse %)))
                               out-network)]
    (assoc post-entry
           :in-network-links
           (into in-network new-in-network)
           :out-of-network-links
           (set/difference out-network new-in-network))))
